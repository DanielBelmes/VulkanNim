...
type VkQueueFlagBits * = enum
  VK_QUEUE_GRAPHICS_BIT = 1  ## Queue supports graphics operations
  VK_QUEUE_COMPUTE_BIT = 2  ## Queue supports compute operations
  VK_QUEUE_TRANSFER_BIT = 4  ## Queue supports transfer operations
  VK_QUEUE_SPARSE_BINDING_BIT = 8  ## Queue supports sparse resource memory management operations
...
type VkMemoryPropertyFlagBits * = enum
  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT = 1  ## If otherwise stated, then allocate memory on device
  VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT = 2  ## Memory is mappable by host
  VK_MEMORY_PROPERTY_HOST_COHERENT_BIT = 4  ## Memory will have i/o coherency. If not set, application may need to use vkFlushMappedMemoryRanges and vkInvalidateMappedMemoryRanges to flush/invalidate host cache
  VK_MEMORY_PROPERTY_HOST_CACHED_BIT = 8  ## Memory will be cached by the host
  VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT = 16  ## Memory may be allocated by the driver when it is required

type VkMemoryHeapFlagBits * = enum
  VK_MEMORY_HEAP_DEVICE_LOCAL_BIT = 1  ## If set, heap represents device memory

type VkAccessFlagBits * = enum
  VK_ACCESS_INDIRECT_COMMAND_READ_BIT = 1  ## Controls coherency of indirect command reads
  VK_ACCESS_INDEX_READ_BIT = 2  ## Controls coherency of index reads
  VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT = 4  ## Controls coherency of vertex attribute reads
  VK_ACCESS_UNIFORM_READ_BIT = 8  ## Controls coherency of uniform buffer reads
  VK_ACCESS_INPUT_ATTACHMENT_READ_BIT = 16  ## Controls coherency of input attachment reads
  VK_ACCESS_SHADER_READ_BIT = 32  ## Controls coherency of shader reads
  VK_ACCESS_SHADER_WRITE_BIT = 64  ## Controls coherency of shader writes
  VK_ACCESS_COLOR_ATTACHMENT_READ_BIT = 128  ## Controls coherency of color attachment reads
  VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT = 256  ## Controls coherency of color attachment writes
  VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT = 512  ## Controls coherency of depth/stencil attachment reads
  VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT = 1024  ## Controls coherency of depth/stencil attachment writes
  VK_ACCESS_TRANSFER_READ_BIT = 2048  ## Controls coherency of transfer reads
  VK_ACCESS_TRANSFER_WRITE_BIT = 4096  ## Controls coherency of transfer writes
  VK_ACCESS_HOST_READ_BIT = 8192  ## Controls coherency of host reads
  VK_ACCESS_HOST_WRITE_BIT = 16384  ## Controls coherency of host writes
  VK_ACCESS_MEMORY_READ_BIT = 32768  ## Controls coherency of memory reads
  VK_ACCESS_MEMORY_WRITE_BIT = 65536  ## Controls coherency of memory writes

type VkBufferUsageFlagBits * = enum
  VK_BUFFER_USAGE_TRANSFER_SRC_BIT = 1  ## Can be used as a source of transfer operations
  VK_BUFFER_USAGE_TRANSFER_DST_BIT = 2  ## Can be used as a destination of transfer operations
  VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT = 4  ## Can be used as TBO
  VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT = 8  ## Can be used as IBO
  VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT = 16  ## Can be used as UBO
  VK_BUFFER_USAGE_STORAGE_BUFFER_BIT = 32  ## Can be used as SSBO
  VK_BUFFER_USAGE_INDEX_BUFFER_BIT = 64  ## Can be used as source of fixed-function index fetch (index buffer)
  VK_BUFFER_USAGE_VERTEX_BUFFER_BIT = 128  ## Can be used as source of fixed-function vertex fetch (VBO)
  VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT = 256  ## Can be the source of indirect parameters (e.g. indirect buffer, parameter buffer)
...
type VkBufferCreateFlagBits * = enum
  VK_BUFFER_CREATE_SPARSE_BINDING_BIT = 1  ## Buffer should support sparse backing
  VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT = 2  ## Buffer should support sparse backing with partial residency
  VK_BUFFER_CREATE_SPARSE_ALIASED_BIT = 4  ## Buffer should support constant data access to physical memory ranges mapped into multiple locations of sparse buffers
...
type VkImageUsageFlagBits * = enum
  VK_IMAGE_USAGE_TRANSFER_SRC_BIT = 1  ## Can be used as a source of transfer operations
  VK_IMAGE_USAGE_TRANSFER_DST_BIT = 2  ## Can be used as a destination of transfer operations
  VK_IMAGE_USAGE_SAMPLED_BIT = 4  ## Can be sampled from (SAMPLED_IMAGE and COMBINED_IMAGE_SAMPLER descriptor types)
  VK_IMAGE_USAGE_STORAGE_BIT = 8  ## Can be used as storage image (STORAGE_IMAGE descriptor type)
  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT = 16  ## Can be used as framebuffer color attachment
  VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT = 32  ## Can be used as framebuffer depth/stencil attachment
  VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT = 64  ## Image data not needed outside of rendering
  VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT = 128  ## Can be used as framebuffer input attachment

type VkImageCreateFlagBits * = enum
  VK_IMAGE_CREATE_SPARSE_BINDING_BIT = 1  ## Image should support sparse backing
  VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT = 2  ## Image should support sparse backing with partial residency
  VK_IMAGE_CREATE_SPARSE_ALIASED_BIT = 4  ## Image should support constant data access to physical memory ranges mapped into multiple locations of sparse images
  VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT = 8  ## Allows image views to have different format than the base image
  VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT = 16  ## Allows creating image views with cube type from the created image
...
type VkFormatFeatureFlagBits * = enum
  VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT = 1  ## Format can be used for sampled images (SAMPLED_IMAGE and COMBINED_IMAGE_SAMPLER descriptor types)
  VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT = 2  ## Format can be used for storage images (STORAGE_IMAGE descriptor type)
  VK_FORMAT_FEATURE_STORAGE_IMAGE_ATOMIC_BIT = 4  ## Format supports atomic operations in case it is used for storage images
  VK_FORMAT_FEATURE_UNIFORM_TEXEL_BUFFER_BIT = 8  ## Format can be used for uniform texel buffers (TBOs)
  VK_FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_BIT = 16  ## Format can be used for storage texel buffers (IBOs)
  VK_FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_ATOMIC_BIT = 32  ## Format supports atomic operations in case it is used for storage texel buffers
  VK_FORMAT_FEATURE_VERTEX_BUFFER_BIT = 64  ## Format can be used for vertex buffers (VBOs)
  VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT = 128  ## Format can be used for color attachment images
  VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BLEND_BIT = 256  ## Format supports blending in case it is used for color attachment images
  VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT = 512  ## Format can be used for depth/stencil attachment images
  VK_FORMAT_FEATURE_BLIT_SRC_BIT = 1024  ## Format can be used as the source image of blits with vkCmdBlitImage
  VK_FORMAT_FEATURE_BLIT_DST_BIT = 2048  ## Format can be used as the destination image of blits with vkCmdBlitImage
  VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT = 4096  ## Format can be filtered with VK_FILTER_LINEAR when being sampled

type VkQueryControlFlagBits * = enum
  VK_QUERY_CONTROL_PRECISE_BIT = 1  ## Require precise results to be collected by the query

type VkQueryResultFlagBits * = enum
  VK_QUERY_RESULT_64_BIT = 1  ## Results of the queries are written to the destination buffer as 64-bit values
  VK_QUERY_RESULT_WAIT_BIT = 2  ## Results of the queries are waited on before proceeding with the result copy
  VK_QUERY_RESULT_WITH_AVAILABILITY_BIT = 4  ## Besides the results of the query, the availability of the results is also written
  VK_QUERY_RESULT_PARTIAL_BIT = 8  ## Copy the partial results of the query even if the final results are not available

type VkCommandBufferUsageFlagBits * = enum
  VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT = 1
  VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT = 2
  VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT = 4  ## Command buffer may be submitted/executed more than once simultaneously

type VkQueryPipelineStatisticFlagBits * = enum
  VK_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_VERTICES_BIT = 1  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_INPUT_ASSEMBLY_PRIMITIVES_BIT = 2  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_VERTEX_SHADER_INVOCATIONS_BIT = 4  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_GEOMETRY_SHADER_INVOCATIONS_BIT = 8  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_GEOMETRY_SHADER_PRIMITIVES_BIT = 16  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_CLIPPING_INVOCATIONS_BIT = 32  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_CLIPPING_PRIMITIVES_BIT = 64  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_FRAGMENT_SHADER_INVOCATIONS_BIT = 128  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_TESSELLATION_CONTROL_SHADER_PATCHES_BIT = 256  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_TESSELLATION_EVALUATION_SHADER_INVOCATIONS_BIT = 512  ## Optional
  VK_QUERY_PIPELINE_STATISTIC_COMPUTE_SHADER_INVOCATIONS_BIT = 1024  ## Optional
...
type VkSparseImageFormatFlagBits * = enum
  VK_SPARSE_IMAGE_FORMAT_SINGLE_MIPTAIL_BIT = 1  ## Image uses a single mip tail region for all array layers
  VK_SPARSE_IMAGE_FORMAT_ALIGNED_MIP_SIZE_BIT = 2  ## Image requires mip level dimensions to be an integer multiple of the sparse image block dimensions for non-tail mip levels.
  VK_SPARSE_IMAGE_FORMAT_NONSTANDARD_BLOCK_SIZE_BIT = 4  ## Image uses a non-standard sparse image block dimensions

type VkSparseMemoryBindFlagBits * = enum
  VK_SPARSE_MEMORY_BIND_METADATA_BIT = 1  ## Operation binds resource metadata to memory

type VkPipelineStageFlagBits * = enum
  VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT = 1  ## Before subsequent commands are processed
  VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT = 2  ## Draw/DispatchIndirect command fetch
  VK_PIPELINE_STAGE_VERTEX_INPUT_BIT = 4  ## Vertex/index fetch
  VK_PIPELINE_STAGE_VERTEX_SHADER_BIT = 8  ## Vertex shading
  VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT = 16  ## Tessellation control shading
  VK_PIPELINE_STAGE_TESSELLATION_EVALUATION_SHADER_BIT = 32  ## Tessellation evaluation shading
  VK_PIPELINE_STAGE_GEOMETRY_SHADER_BIT = 64  ## Geometry shading
  VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT = 128  ## Fragment shading
  VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT = 256  ## Early fragment (depth and stencil) tests
  VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT = 512  ## Late fragment (depth and stencil) tests
  VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT = 1024  ## Color attachment writes
  VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT = 2048  ## Compute shading
  VK_PIPELINE_STAGE_TRANSFER_BIT = 4096  ## Transfer/copy operations
  VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT = 8192  ## After previous commands have completed
  VK_PIPELINE_STAGE_HOST_BIT = 16384  ## Indicates host (CPU) is a source/sink of the dependency
  VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT = 32768  ## All stages of the graphics pipeline
  VK_PIPELINE_STAGE_ALL_COMMANDS_BIT = 65536  ## All stages supported on the queue

type VkCommandPoolCreateFlagBits * = enum
  VK_COMMAND_POOL_CREATE_TRANSIENT_BIT = 1  ## Command buffers have a short lifetime
  VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT = 2  ## Command buffers may release their memory individually

type VkCommandPoolResetFlagBits * = enum
  VK_COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT = 1  ## Release resources owned by the pool

type VkCommandBufferResetFlagBits * = enum
  VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT = 1  ## Release resources owned by the buffer

type VkSampleCountFlagBits * = enum
  VK_SAMPLE_COUNT_1_BIT = 1  ## Sample count 1 supported
  VK_SAMPLE_COUNT_2_BIT = 2  ## Sample count 2 supported
  VK_SAMPLE_COUNT_4_BIT = 4  ## Sample count 4 supported
  VK_SAMPLE_COUNT_8_BIT = 8  ## Sample count 8 supported
  VK_SAMPLE_COUNT_16_BIT = 16  ## Sample count 16 supported
  VK_SAMPLE_COUNT_32_BIT = 32  ## Sample count 32 supported
  VK_SAMPLE_COUNT_64_BIT = 64  ## Sample count 64 supported

type VkAttachmentDescriptionFlagBits * = enum
  VK_ATTACHMENT_DESCRIPTION_MAY_ALIAS_BIT = 1  ## The attachment may alias physical memory of another attachment in the same render pass

type VkStencilFaceFlagBits * = enum
  VK_STENCIL_FACE_FRONT_BIT = 1  ## Front face
  VK_STENCIL_FACE_BACK_BIT = 2  ## Back face
  VK_STENCIL_FACE_FRONT_AND_BACK = 0x00000003  ## Front and back faces

type VkDescriptorPoolCreateFlagBits * = enum
  VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT = 1  ## Descriptor sets may be freed individually

type VkDependencyFlagBits * = enum
  VK_DEPENDENCY_BY_REGION_BIT = 1  ## Dependency is per pixel region
...
type VkSubgroupFeatureFlagBits * = enum
  VK_SUBGROUP_FEATURE_BASIC_BIT = 1  ## Basic subgroup operations
  VK_SUBGROUP_FEATURE_VOTE_BIT = 2  ## Vote subgroup operations
  VK_SUBGROUP_FEATURE_ARITHMETIC_BIT = 4  ## Arithmetic subgroup operations
  VK_SUBGROUP_FEATURE_BALLOT_BIT = 8  ## Ballot subgroup operations
  VK_SUBGROUP_FEATURE_SHUFFLE_BIT = 16  ## Shuffle subgroup operations
  VK_SUBGROUP_FEATURE_SHUFFLE_RELATIVE_BIT = 32  ## Shuffle relative subgroup operations
  VK_SUBGROUP_FEATURE_CLUSTERED_BIT = 64  ## Clustered subgroup operations
  VK_SUBGROUP_FEATURE_QUAD_BIT = 128  ## Quad subgroup operations
...
type VkPeerMemoryFeatureFlagBits * = enum
  VK_PEER_MEMORY_FEATURE_COPY_SRC_BIT = 1  ## Can read with vkCmdCopy commands
  VK_PEER_MEMORY_FEATURE_COPY_DST_BIT = 2  ## Can write with vkCmdCopy commands
  VK_PEER_MEMORY_FEATURE_GENERIC_SRC_BIT = 4  ## Can read with any access type/command
  VK_PEER_MEMORY_FEATURE_GENERIC_DST_BIT = 8  ## Can write with and access type/command

type VkMemoryAllocateFlagBits * = enum
  VK_MEMORY_ALLOCATE_DEVICE_MASK_BIT = 1  ## Force allocation on specific devices

type VkDeviceGroupPresentModeFlagBitsKHR * = enum
  VK_DEVICE_GROUP_PRESENT_MODE_LOCAL_BIT_KHR = 1  ## Present from local memory
  VK_DEVICE_GROUP_PRESENT_MODE_REMOTE_BIT_KHR = 2  ## Present from remote memory
  VK_DEVICE_GROUP_PRESENT_MODE_SUM_BIT_KHR = 4  ## Present sum of local and/or remote memory
  VK_DEVICE_GROUP_PRESENT_MODE_LOCAL_MULTI_DEVICE_BIT_KHR = 8  ## Each physical device presents from local memory
